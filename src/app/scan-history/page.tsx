...
export default function ScanHistoryPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white p-8 font-sans selection:bg-[#00ff9d]/30">
      {/* Header */}
      <div className="mb-12 max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black italic tracking-tighter uppercase mb-2">
            SCAN<span className="text-[#00ff9d]">ARCHIVE</span>
          </h1>
        ...
        <div className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-xl">
          <div className="text-right">
            ...
            <p className="text-xl font-black italic text-[#00ff9d] leading-none">
              {allScans.length}
            </p>
          </div>
      ...
      <div className="mb-8 max-w-7xl mx-auto flex flex-col lg:flex-row gap-4">
        ...
        <div className="flex-1 relative group">
          <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-500 group-focus-within:text-[#00ff9d] transition-colors" />
          <input
            type="text"
            placeholder="SEARCH NEURAL RECORDS..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-12 pr-4 py-4 bg-white/5 border border-white/5 rounded-2xl focus:outline-none focus:ring-1 focus:ring-[#00ff9d]/50 focus:bg-white/10 transition-all font-mono text-xs uppercase tracking-widest placeholder:text-gray-700"
          />
        </div>

        {/* Filter Buttons */}
        <div className="flex gap-2 p-1.5 bg-white/5 rounded-2xl border border-white/5 h-fit">
          {[
            { id: "all", label: "ALL UNITS", color: "text-white" },
            { id: "healthy", label: "HEALTHY", color: "text-[#00ff9d]" },
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-xl font-black italic text-[10px] tracking-[0.1em] transition-all ${
                filter === btn.id
        ...
        {/* Range Filter */}
        <button className="px-6 py-4 bg-white/5 border border-white/5 text-[10px] font-black italic tracking-widest text-[#00ff9d] rounded-2xl hover:bg-[#00ff9d]/10 transition-all flex items-center gap-3">
